...
while(cont == True):
    ...
    def positiveresponse():
        ...
        def savefile(): 
            ...
            # webbrowser.open(f'ftp://attendance/Attendance_{dts}.html')
            with open('Attendance.csv', 'r+') as f:
                showcsv = f.readlines()
                f.close()
                ...
                root2 = Tk()
                root2.geometry("300x800")
                Label(root2, text=" DATA FINAL ::::::::").pack()
                Label(root2, text=showcsv).pack()
                Button(root2, text="CLOSE AND QUIT ", command=exitfunc).pack()
                root2.mainloop() 
        ...
        Label(root1, text="YOU CAN CHECK REPORTS IN ATTENDANCE FOLDER ").place(x=10, y=15)
        
        Label(root1, text="CLICK YES TO MARK MORE ATTENDANCES, NO TO EXIT AND SAVE THE FILE").place(x=10, y=30)
        Radiobutton(root1, text="YES", value="yes", variable=var1, command=root1.destroy).place(x=10, y=60)
        Radiobutton(root1, text="NO", value="no", variable=var1, command=savefile).place(x=10, y=80)
    ...
    Label(root, text="CLICK YES if you are ready to mark the attendance.Be in front of Camera\n and rest will be done ").place(x=2, y=10)
    Button(root, text="YES", command=positiveresponse).place(x=2, y=40)
